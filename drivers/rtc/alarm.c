...
static struct rtc_device *alarm_virtual_rtc_dev;
struct rtc_wkalrm virtual_alarm;
#endif
				...
				struct class_interface *class_intf)
{
	...
	if (alarm_rtc_dev) {
#ifdef CONFIG_RTC_DRV_VIRTUAL
		if(alarm_virtual_rtc_dev){
			err = -EBUSY;
			goto err1;
		}

		alarm_virtual_rtc_dev = rtc;
		printk("%s, Set virtual rtc\n", __func__); //, alarm_virtual_rtc_dev->name);
		mutex_unlock(&alarm_setrtc_mutex);
		return 0;
#endif
		err = -EBUSY;
