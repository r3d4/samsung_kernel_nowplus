...
static DECLARE_WORK(queue_rtc_work, rtc_work);
...
#endif
	/*queue RTC work*/
	 queue_work(rtc_work_queue, &queue_rtc_work);
...
static int __devinit twl_rtc_probe(struct platform_device *pdev)
{
	...
	/* implementation of RTC work queue */
 	rtc_work_queue = create_singlethread_workqueue("rtc_work");
        if (rtc_work_queue == NULL) {
                ret = -ENOMEM;
                goto out1;
        }
...
static int __devexit twl_rtc_remove(struct platform_device *pdev)
{
	...
	rtc_device_unregister(rtc);
	destroy_workqueue(rtc_work_queue);
