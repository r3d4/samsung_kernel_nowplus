...
static void ear_adc_caculrator(unsigned long arg)
{
	...
	if (state)
	{
		...
		twl_i2c_write_u8( TWL4030_MODULE_PM_RECEIVER, 0x20, VINTANA2_DEV_GRP );
#if 0
   		if (!di->usb3v1) {
		    printk("%s:%d \n",__FUNCTION__,__LINE__);
		    return;
		...
		//di->usb3v1 = regulator_get(this_dev, "usb3v1");
		struct regulator *usb3v1 = regulator_get(this_dev, "usb3v1");
		if (!usb3v1) {
		    printk("%s:%d \n",__FUNCTION__,__LINE__);
		    return;
		}

		ret = regulator_enable( usb3v1 );
		if ( ret )
