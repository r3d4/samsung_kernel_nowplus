...
#define OMAP_MMC_DISABLED_TIMEOUT	1000	
				  ...
				  int vdd)
{
	...
	printk("mmc::omap_hsmmc_1_set_power: power_on = %d\n", power_on);
...
static int omap_hsmmc_suspend(struct device *dev)
{
	...
	if (host) {
		...
		mmc_host_enable(host->mmc);
		if (ret == 0) {
#ifdef WA_FCLK_ENABLE                    // !!!TI WA keep fclk enabled during register access
    		if(host->dpm_state != ENABLED)	
			{
		 	printk("FCLK was disabled \n");
    			fclk_en_sus = 1;
    			clk_enable(host->fclk);
			}
#endif
			...
			{
                fclk_en_sus = 0 ;
                clk_disable(host->fclk);           // done to make sure that fclk is disabled b4 suspend "double check"
			}
#endif	
			if (host->got_dbclk)
				clk_disable(host->dbclk);
