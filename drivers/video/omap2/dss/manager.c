...
static ssize_t refresh_store(struct kobject *k, struct kobj_attribute *, const char *buf, size_t n);
static struct kobj_attribute refresh_attr = __ATTR(lcd_refresh_rate, 0644, refresh_show, refresh_store);
#endif
...
#else
		if((plane != 0) && nDropCount &&(isCamera)) {
			dispc_enable_plane(plane, 0);
			nDropCount--;
		} else {  
			dispc_enable_plane(plane, 1);
...
static int omap_dss_mgr_apply(struct omap_overlay_manager *mgr)
{
	...
	if(change_refresh_rate && first_init) {
		first_init = 0;
