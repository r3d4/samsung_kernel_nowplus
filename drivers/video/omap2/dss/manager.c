...
//To prevent sync lost during booting
#ifdef LCD_REFRESH_CHANGE
unsigned short int refresh_rate = OMAP_DSS_FR_60HZ;
unsigned short int refresh_count = 0;
unsigned short change_refresh_rate;
static ssize_t refresh_show(struct kobject *, struct kobj_attribute *, char *);
static ssize_t refresh_store(struct kobject *k, struct kobj_attribute *,
                          const char *buf, size_t n);
static struct kobj_attribute refresh_attr =
        __ATTR(lcd_refresh_rate, 0644, refresh_show, refresh_store);
#endif
...
#endif
#ifdef RSZ_720
        if((plane != 0) && (is720p) && nDropCount)
        {
               dispc_enable_plane(plane, 0); 
               nDropCount--;
//               printk("VIK_DBG:Drop frame %d\n",nDropCount);
        }
        else
        {  
...
#ifdef LCD_REFRESH_CHANGE
//To prevent Sync lost on booting
	if((change_refresh_rate) && (first_init == 1)) {
		first_init = 0;
		reduce_refresh_rate(0);
		refresh_rate = OMAP_DSS_FR_60HZ;
		DSSINFO("Bootup time Increasing the Refresh Rate to 60HZ\n");
	}
	
	if (refresh_rate == OMAP_DSS_FR_52HZ_SUSPEND)
	{
		refresh_count--;
		if (!refresh_count)
		{
			reduce_refresh_rate(0);
			refresh_rate = OMAP_DSS_FR_60HZ;
//			DSSINFO("Increasing the Refresh Rate to 60HZ\n");
		}
	}
...
#ifdef LCD_REFRESH_CHANGE
static ssize_t refresh_show(struct kobject *kobj, struct kobj_attribute *attr,
                         char *buf)
{
        if (attr == &refresh_attr)
                          ...
                          const char *buf, size_t n)
{
        ...
        if (sscanf(buf, "%hu", &value) != 1) {
                printk(KERN_ERR "refresh_store: Invalid value\n");
                return -EINVAL;
        }

        if (attr == &refresh_attr) {
                change_refresh_rate = value;    //Added to increase refresh rate after booting
        }
