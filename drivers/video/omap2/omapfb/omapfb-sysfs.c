...
static ssize_t store_progress(struct device *dev, struct device_attribute *attr,
		const char *buf, size_t count)
{
	int ret = 0;
	char *after;
	unsigned long value = simple_strtoul(buf, &after, 10);	
	unsigned int size = (after - buf);
	extern int draw_progress;
...
static struct device_attribute omapfb_attrs[] = {
	...
	__ATTR(progress, S_IWUSR, NULL, store_progress),
};
