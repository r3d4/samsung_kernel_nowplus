...
#undef TEMPORARY_FIX_FOR_SGX_BLOCKING_RETENTION
#ifndef TEMPORARY_FIX_FOR_SGX_BLOCKING_RETENTION
int stop_vsync_disable = 0;
EXPORT_SYMBOL(stop_vsync_disable);
#endif
int draw_progress = 1;
...
#if 1
	memset(addr, 0x0, (800 * 480 * 4)); // speed optimiztion
	msleep(300); // waiting update
#else
	for (y = 0; y < h *2; y++) {
		for (x = 0; x < w; x++) {
		...
		enum omap_color_mode *mode)
{
	...
	case 24:
		...
		break;
	case 32:
#if 1 //def CONFIG_MACH_OSCAR //MIG_c[[
		dssmode = OMAP_DSS_COLOR_ARGB32;
#else
		dssmode = OMAP_DSS_COLOR_RGB24U;
#endif //MIG]]
...
#if 0
#ifdef CONFIG_MACH_NOWPLUS // kyNam_101007_ trouble shooting : after sleep(App is running) and wakeup, App-Screen has seen shortly -> screen buffer clear 
	{
		void __iomem *addr = fbi->screen_base;
		DBG( "%s:%d(%s), screen buffer clear, fbi=0x%x\n", __FUNCTION__, __LINE__, __FILE__, fbi );
		memset(addr, 0x0, (480 * 800 * 4L *2L )); // 1pixel=4bytes, 2screen // speed optimiztion
		msleep(200); // waiting update // kyNam_101018_ ArcherFroyo_KOR : Sleep while App displaying -> Wakeup -> App Screen have seen before Lock Screen(GlassLock)
	}	
#endif
#endif
