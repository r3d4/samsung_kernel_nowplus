...
//extern void omap_console_activity(int num);
...
/*yangyu 20100130 modified for gps uart problem: open->transmit->close, then reopen cannot use uart*/
static inline void
serial_omap_configure_close_xonxoff
		(struct uart_omap_port *up, struct ktermios *termios)
{
	unsigned char efr = 0;
	...
	serial_out(up, UART_XON1, 0x0);
    serial_out(up, UART_XOFF1, 0x0);
			...
			struct ktermios *old)
{
	...
	if (termios->c_iflag & (IXON | IXOFF))
        ...
        /*yangyu 20100130 modified for gps uart problem: open->transmit->close, then reopen cannot use uart*/
            ...
            serial_omap_configure_close_xonxoff(up, termios);
...
static inline void wait_for_xmitr(struct uart_omap_port *up)
{
	unsigned int status, tmout = 10000;
       /* ping for console activity if uart is idled */
      // omap_console_activity(up->pdev->id);
