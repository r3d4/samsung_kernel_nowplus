...
#if (CONFIG_MACH_SAMSUNG_ARCHER || CONFIG_MACH_OSCAR)
		gpio_set_value(OMAP_GPIO_USB_SEL, 1);
...
#if (CONFIG_MACH_SAMSUNG_ARCHER || CONFIG_MACH_OSCAR)
		if(!usbsel){
			gpio_set_value(OMAP_GPIO_USB_SEL, 0);
...
#if (CONFIG_MACH_SAMSUNG_ARCHER || CONFIG_MACH_OSCAR)
	if(gpio_request(OMAP_GPIO_USB_SEL, "Phone_VBUS_enable")<0) {
		printk(KERN_ERR "Failed to get OMAP_GPIO_USB_SEL pin \n");
		return;
	}
	printk("RANJ ** MODEM USB SELECT PIN = %d configured as output \n", (int)OMAP_GPIO_USB_SEL);
	gpio_direction_output(OMAP_GPIO_USB_SEL, 0);
#endif
	microusbic_i2c_init();
