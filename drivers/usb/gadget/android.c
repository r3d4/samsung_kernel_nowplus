...
#if ADB_ENABLE_BY_PARAM
extern void (*sec_set_param_value)(int idx, void *value);

int adb_enabled_param = 1;

static int __init __adb_enabled(char *str)
{
	get_option(&str, &adb_enabled_param);
	return 0;
}

early_param("adb_enabled", __adb_enabled);
#endif
...
void android_enable_function(struct usb_function *f, int enable)
{
	...
	if (!!f->hidden != disable) {
		...
		if (!strcmp(f->name, "adb")) {
			...
			{
				sec_set_param_value(__PARAM_INT_12, &enable);
			}
#endif
#if !defined(CONFIG_USB_IAD)
			struct usb_function		*func;
