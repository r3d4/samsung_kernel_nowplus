	...
	if ((atmel_msg[0]==2)||(atmel_msg[0]==3))
	{
		...
		if((id2.press+id3.press)!= 2)
		{
			...
			if( ( atmel_msg[1] & 0xC0 ) == 0xC0  ) 
			{
				...
				if ( (id2.press + id3.press) == 2 )
				{
					...
					REPORT( 1, size, x, y );
                                        ...
                                        #endif
                                        if(debug_print_on)
        					printk("[TSP] detect one with new detect id=%d, x=%d, y=%d\n", atmel_msg[0] , x, y );
				...
				else
				{
					...
					REPORT( 1, size, x, y );
					if(debug_print_on)
        					printk("[TSP] one detect - id=%d, x=%d, y=%d\n", atmel_msg[0] , x, y );
				}
			}
			...
			else if( ((atmel_msg[1] & 0x20 ) == 0x20))   
			{
				REPORT( 0, 1, x, y );
				if(debug_print_on)
        				printk("[TSP] id=%d, x=%d, y=%d - release\n", atmel_msg[0] , x, y );
		...
		else if( id2.press && id3.press )
		{
			...
			if( ((atmel_msg[1] & 0x20 ) == 0x20))   // release
			{
				...
				if(debug_print_on)	
        				printk("[TSP] two input & one release\tid=2, x=%d, y=%d, id=3, x=%d, y=%d release id=%d\n", id2.x, id2.y ,id3.x, id3.y,  atmel_msg[0] );
