...
extern int nowplus_enable_touch_pins(int enable);
/**
						...
						struct synaptics_rmi4_fn *rfi)
{
	...
	if (retval != QUERY_LEN)
		...
		pdata->sensor_max_x = 480; /*((data[6] & MASK_8BIT) << 0) |
						((data[7] & MASK_4BIT) << 8);*/
		pdata->sensor_max_y = 800; /*((data[8] & MASK_5BIT) << 0) |
						((data[9] & MASK_4BIT) << 8);*/
	}
	return retval;
}
	...
	(struct i2c_client *client, const struct i2c_device_id *dev_id)
{
	...
	if (platformdata->regulator_en) {
		rmi4_data->regulator = regulator_get(&client->dev, "vaux2");
	...
	input_set_abs_params(rmi4_data->input_dev, ABS_MT_TOUCH_MAJOR, 0,
						MAX_TOUCH_MAJOR, 0, 0);
	input_set_abs_params(rmi4_data->input_dev, ABS_MT_WIDTH_MAJOR, 0, 15, 0, 0);
...
static void synaptics_rmi4_early_suspend(struct early_suspend *h)
{
	...
	synaptics_rmi4_suspend(ts->i2c_client, PMSG_SUSPEND);
}
...
static void synaptics_rmi4_late_resume(struct early_suspend *h)
{
	struct synaptics_rmi4_data *ts;
	ts = container_of(h, struct synaptics_rmi4_data, early_suspend);
	synaptics_rmi4_resume(ts->i2c_client);
}
#endif
