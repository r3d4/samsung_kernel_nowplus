/*============================================================================
Filename : $Workfile: 602240_demo_code.c $
Project  : QT602240 Host Example Code
Purpose  : To be controlled by Host processor
------------------------------------------------------------------------------
Version Number : $0.1 Based on AVR32 Revision 8 $
Last Updated   : $Date: 2009-06-18 17:48:07+09:00 $
Updated By     : $Author: goodman $
------------------------------------------------------------------------------
...
// so please don't change place this function is called
    if(g_version < 0x15)
    {
        ...
        switch(g_model)
        {
            case ARCHER_KOR_SK:
            {
                #if ((CONFIG_ARCHER_HW_REV <= ARCHER_REV11) && (CONFIG_ARCHER_HW_REV >= ARCHER_REV10))            
    ...
    Atmel_model_type model_type )
{
    switch(model_type)
    {
        case ARCHER_KOR_SK:
        {
            ...
            #if (CONFIG_ARCHER_HW_REV == ARCHER_REV12) || (CONFIG_ARCHER_HW_REV == ARCHER_REV13) || (CONFIG_ARCHER_HW_REV == ARCHER_REV20)
        	power_config->idleacqint = 64; 
    ...
    Atmel_model_type model_type )
{
    switch(model_type)
    {
		...
		touchscreen_config->jumplimit = 18;
            ...
            #if (CONFIG_ARCHER_HW_REV == ARCHER_REV12)
//            touchscreen_config->blen = 17;
        	touchscreen_config->yloclip = 100;        
            #elif (CONFIG_ARCHER_HW_REV == ARCHER_REV13)
//            touchscreen_config->blen = 49;
        	touchscreen_config->yloclip = 0;
        	#elif (CONFIG_ARCHER_HW_REV == ARCHER_REV20)
//            touchscreen_config->blen = 49;
        	touchscreen_config->yloclip = 0; 
            #endif
            #elif ((CONFIG_ARCHER_HW_REV <= ARCHER_REV11) && (CONFIG_ARCHER_HW_REV >= ARCHER_REV10))
            touchscreen_config->orientate = 1;
    ...
    Atmel_model_type model_type )
{
    switch(model_type)
    {
        case ARCHER_KOR_SK:
        {
            ...
            #if (CONFIG_ARCHER_HW_REV == ARCHER_REV20)       
            cte_config->mode = 2;
            cte_config->idlegcafdepth = 4;
            ...
            cte_config->idlegcafdepth = 4;
            cte_config->actvgcafdepth = 8;
            #elif (CONFIG_ARCHER_HW_REV == ARCHER_REV12)
            cte_config->mode = 2;
