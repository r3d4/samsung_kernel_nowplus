...
#define KEY_PROC_FS
#define _ENABLE_LONG_KEY_TIMEOUT_
...
static int twl4030_read_kp_matrix_state(struct twl4030_keypad *kp, u16 *state)
{
    ...
    printk("twl4030_read_kp_matrix_state (%d) (%d) (%d) \n", new_state[0], new_state[1], new_state[2]);
    
#ifdef CONFIG_SAMSUNG_KERNEL_DEBUG_USER// klaatu
...
static int __devinit twl4030_kp_program(struct twl4030_keypad *kp)
{
...
#ifdef _ENABLE_LONG_KEY_TIMEOUT_
	reg &= ~KEYP_CTRL_TOE_EN;
	reg |= KEYP_CTRL_LK_EN | KEYP_CTRL_TOLE_EN;
#endif
	...
	if (twl4030_kpwrite_u8(kp, (i && 0xFF), KEYP_LONG_KEY) < 0)
		return -EIO;

	/* Set timeout period to 5 s */
	i = KEYP_PERIOD_US(5000000, PTV_PRESCALER);
