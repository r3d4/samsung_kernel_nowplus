	...
	struct regulation_constraints *constraints)
{
	...
	if (ret != 0) {
		printk(KERN_ERR "%s: no constraints set for %s\n",
			       __func__, name);
		goto out;
	}
		...
		if (ret == -EINVAL || ret == 0) {
			...
			if (rdev->desc->ops->enable) {
				...
				if (ret < 0)
					return ret;
				/*
				 * TI CSR OMAPS00222879: "MP3 playback current 
				 * consumption is very high"
				 * This is a workaround to get the proper battery ADC values.
		 		 */
				if (!strcmp(rdev_get_name(rdev), "VUSB1V5"))
					twl_i2c_write_u8(0x00, 0, 0xFD/*PHY_PWR_CTRL*/);
			} else {
				return -EINVAL;
			}
		} else if (ret < 0) {
...
static int _regulator_disable(struct regulator_dev *rdev)
{
	...
	if (rdev->use_count == 1 &&
		 ...
		 */
		if (!strcmp(rdev_get_name(rdev), "VUSB1V5"))
				twl_i2c_write_u8(0x00, 1, 0xFD/*PHY_PWR_CTRL*/);
