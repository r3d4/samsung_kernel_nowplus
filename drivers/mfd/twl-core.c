...
int twl_i2c_write(u8 mod_no, u8 *value, u8 reg, unsigned num_bytes)
{
	...
	if(mod_no == TWL4030_MODULE_AUDIO_VOICE)
	{
        ...
        printk("TSKIM -vociecall mode : mod_no %d -  %x regiter configured to %x\n", mod_no, reg, value[1]);
	
		if(reg == 0x25 && value[1] == 0)
	      {
	      		if(twl4030_get_voicecall_state())
	      		{
	      		     printk("TSKIM -vociecall mode : prevent %d -  %x regiter configured to %x\n", mod_no, reg, value[1]);
	      		     return 0;
	      		}
	      }
	...
	*value = twl_map[mod_no].base + reg;
	...
	if(mod_no == 0x1 && reg ==0x01)
		codec_pdz_val = msg->buf[1] & 0x02;
...
u8 twl4030_read_codec_pdz(void)
{
	return codec_pdz_val;
}
EXPORT_SYMBOL(twl4030_read_codec_pdz);
/**
			...
			struct regulator_init_data usb_fixed = {
				...
				.constraints.valid_ops_mask =
					...
					| REGULATOR_CHANGE_STATUS,
                        ...
                        usb1v5[1].dev = pdata->usb->batt_dev;
                        usb1v8[1].dev = pdata->usb->batt_dev;
                        usb3v1[1].dev = pdata->usb->batt_dev;

                        usb1v5[2].dev = pdata->usb->charger_dev;
                        usb1v8[2].dev = pdata->usb->charger_dev;
                        usb3v1[2].dev = pdata->usb->charger_dev;

                        usb1v5[3].dev = pdata->usb->switch_dev;
                        usb1v8[3].dev = pdata->usb->switch_dev;
                        usb3v1[3].dev = pdata->usb->switch_dev;
