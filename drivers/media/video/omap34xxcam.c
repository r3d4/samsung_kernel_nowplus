		      ...
		      struct v4l2_fract *best_ival)
{
  ...
  /* modified for image capture, first enumerate sensor and then isp */
	...
	rval = isp_s_fmt_cap(isp, best_pix, pix,
		vdev->vdev_sensor_config.isp_if, vdev->vfd->index);
	if (rval)
		return rval;
...
static int vidioc_qbuf(struct file *file, void *_fh, struct v4l2_buffer *b)
{
  ...
  int rval = 0;
  int recnt = 0;
//  printk("NCB_DBG::omap34xxcam::QBUF\n");
  while(recnt < 10)
  {
...
//	printk("NCB_DBG::omap34xxcam::DQBUF::%d\n",rval);
	return rval;
}
...
static int omap34xxcam_device_register(struct v4l2_int_device *s)
{
    ...
    printk(KERN_ERR "***bsd_cam : entered %s \n", __func__);
