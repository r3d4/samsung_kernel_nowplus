...
int videobuf_waiton(struct videobuf_buffer *vb, int non_blocking, int intr)
{
	...
	if (intr)
	{
#if 0 //JWWON (for remove deadlock)
		 ...
		 struct v4l2_requestbuffers *req)
{
	...
	req->count = retval;
//	retval = 0; //Commented by Venu in Code Review
				      ...
				      size_t count, loff_t *ppos)
{
	...
	if(q->read_buf)kfree(q->read_buf);
	q->read_buf = NULL;
	return retval;
}
