...
#define DROP_FRAMES_COUNT 6   //RSZ_720
			  ...
			  unsigned int *count, unsigned int startindex)
{
	...
	for (i = 0; i < *count; i++) {
		...
		omap_vrfb_setup(&vout->vrfb_context[i],
				vout->smsshado_phy_addr[i], width,
				height, vout->bpp, yuv_mode,
				vout->rotation);
	}
	return 0;
}
			    ...
			    enum v4l2_field field)
{
	...
	vout->queued_buf_addr[vb->i] = (u8 *) vout->vrfb_context[vb->i].paddr[0];
			...
			struct v4l2_format *f)
{
	...
	if (vout->pix.height * vout->pix.width == VID_MAX_WIDTH * 720)
	{
		...
		flg_720 = VIDEO_720_ENABLE;
#ifdef CONFIG_PM
		/*Fix for 720p seek (black screen) problem.  Moved the frequncy boosting
		  from streamon to s_fmt */
		omap_pm_set_min_mpu_freq(((vout->vid_dev)->v4l2_dev).dev, VDD1_OPP6_FREQ);
#endif
	}
	else
	{
...
static int vidioc_streamon(struct file *file, void *fh, enum v4l2_buf_type i)
{
	...
	if (ret)
	{
		printk("%s:videobuf_streamon err=%d\n",__func__,ret);
		goto streamon_err;
	}

	if (list_empty(&vout->dma_queue)) {
	
		printk("%s:dma_queue empty\n",__func__);
		ret = -EIO;
		goto streamon_err1;
	}
	...
	*		  
	*/
	omap_pm_set_min_mpu_freq(((vout->vid_dev)->v4l2_dev).dev, VDD1_OPP6_FREQ); //temp... original VDD1_OPP3_FREQ
#endif
	...
	for (j = 0; j < ovid->num_overlays; j++) {
		...
		if (ovl->manager && ovl->manager->device) {
			...
			info.p_uv_addr = uv_addr;
			if (ovl->set_overlay_info(ovl, &info)) {
				printk("%s: set_overlay_info error \n",__func__);
				ret = -EINVAL;
				goto streamon_err1;
			}
		}
...
#ifdef CONFIG_PM
	omap_pm_set_min_mpu_freq(((vout->vid_dev)->v4l2_dev).dev, VDD1_FREQ_MIN); //temp
