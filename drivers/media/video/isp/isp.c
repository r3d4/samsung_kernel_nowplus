...
#ifndef ZEUS_CAM					
		| IRQ0ENABLE_CSIA_IRQ 
		| IRQ0ENABLE_CSIB_LCM_IRQ
		| IRQ0ENABLE_CSIB_LC0_IRQ 
...
static irqreturn_t isp_isr(int irq, void *_pdev)
{
	...
	/* Workaround for Preview flickering */
	if((prev_irqstatus == irqstatus) && (irqstatus == CCDC_VD0))
	{
		irqstatus = HS_VS;
	}
	prev_irqstatus = irqstatus;
...
#define ISP_STATISTICS_BUSY			\
	()
#define ISP_STOP_TIMEOUT	msecs_to_jiffies(100)   //sasken Reduced the timeout from 1000 to 100 ms
...
#ifndef ZEUS_CAM
	timeout = jiffies + ISP_STOP_TIMEOUT;
	while (isppreview_busy(&isp->isp_prev)
#ifdef ZEUS_720P		
		   || ispresizer_busy(&isp->isp_res)
#endif		
		   || isp_af_busy(&isp->isp_af)
   ...
   */  
	...
	if (suspend) {
		...
		* stopping of the LSC sw reset the isp */
		//OMAP3: ISP: Reset ISP if ispccdc_enable_lsc() fails.
		//This is fixing the wrong applying of LSC.
		
		//TI Gerrit patch Ia2252f1e	
...
struct device *isp_get(void)
{
	...
	if (!pdev)
	{
		printk(KERN_ERR "**bsd_cam : %s : pdev is NULL\n", __func__);
		return NULL;
	}
	isp = platform_get_drvdata(pdev);
	printk("isp_get: old %d\n", isp->ref_count);
	mutex_lock(&(isp->isp_mutex));
	if (isp->ref_count == 0) {
		ret_err = isp_enable_clocks(&pdev->dev);
		if (ret_err) {
			printk(KERN_ERR "**bsd_cam : %s : ret_err=%d\n", __func__, ret_err);
			goto out_err;
		}
...
static int isp_remove(struct platform_device *pdev)
{
	...
	for (i = 0; i <= OMAP3_ISP_IOMEM_CSI2PHY2; i++) {
   	...
   	printk(KERN_ERR "***bsd_cam : %s :making omap3isp_pdev=NULL\n",__func__);
	omap3isp_pdev = NULL;
...
static int isp_probe(struct platform_device *pdev)
{
	...
	isp->ref_count = 0;
	omap3isp_pdev = pdev;
	printk(KERN_ERR "***bsd_cam : %s : omap3isp_pdev=%p\n",__func__,omap3isp_pdev);
	...
	if (IS_ERR(isp->iommu)) {
		...
		isp->iommu = NULL;
		printk(KERN_ERR "***bsd_cam : %s : iommu error\n",__func__);
	}
#ifdef CONFIG_VIDEO_OMAP34XX_ISP_DEBUG_FS
	isp_dfs_setup(isp);
#endif
	isp_put();
	if (!isp->iommu) {
		printk(KERN_ERR "***bsd_cam : %s : isp->iommu =%p \n",__func__, isp->iommu);
		goto out_iommu_get;
