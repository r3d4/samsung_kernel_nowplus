...
static int reg_resize_qcif_index;
static int reg_resize_ivs_qcif_index;
static int reg_resize_cif_index;
static int reg_resize_ivs_cif_index;
static int reg_resize_qvga_index;
static int reg_resize_ivs_qvga_index;
static int reg_resize_vga_index;
...
static int reg_fps_7_index;
static int reg_fps_10_index;
static int reg_fps_12_index;
static int reg_fps_15_index;
...
static int s5ka3dfx_set_fps(void)
{
	...
	if(sensor->state != S5KA3DFX_STATE_CAPTURE) {
		...
		switch(sensor->fps) {
		case 15:
			for (i = 0; i < reg_fps_15_index; i++) {
				if(s5ka3dfx_set_data(client, reg_fps_15_table[i]))
					goto fps_fail;
			}            
			break;

		case 12:
			for (i = 0; i < reg_fps_12_index; i++) {
				if(s5ka3dfx_set_data(client, reg_fps_12_table[i]))
					goto fps_fail;
			}            
			break;

		case 10:
			for (i = 0; i < reg_fps_10_index; i++) {
				if(s5ka3dfx_set_data(client, reg_fps_10_table[i]))
					goto fps_fail;
			}            
			break;

		case 7:
			for (i = 0; i < reg_fps_7_index; i++) {
				if(s5ka3dfx_set_data(client, reg_fps_7_table[i]))
					goto fps_fail;
			}            
			break;   
...
static int s5ka3dfx_set_table (void)
{
	...
	reg_resize_qcif_index = 0;
	reg_resize_ivs_qcif_index = 0;		
	reg_resize_cif_index = 0;
	reg_resize_ivs_cif_index = 0;		
	reg_resize_qvga_index = 0;
	reg_resize_ivs_qvga_index = 0;		
	reg_resize_vga_index = 0;
	reg_wb_auto_index = 0;
	...
	reg_resize_qcif_index = sizeof(reg_resize_qcif_table)/sizeof(u32);
	reg_resize_ivs_qcif_index = sizeof(reg_resize_ivs_qcif_table)/sizeof(u32);
	reg_resize_cif_index = sizeof(reg_resize_cif_table)/sizeof(u32);
	reg_resize_ivs_cif_index = sizeof(reg_resize_ivs_cif_table)/sizeof(u32);
	reg_resize_qvga_index = sizeof(reg_resize_qvga_table)/sizeof(u32);
	reg_resize_ivs_qvga_index = sizeof(reg_resize_ivs_qvga_table)/sizeof(u32);
	reg_resize_vga_index = sizeof(reg_resize_vga_table)/sizeof(u32);
	...
	reg_fps_7_index = sizeof(reg_fps_7_table)/sizeof(u32);
	reg_fps_10_index = sizeof(reg_fps_10_table)/sizeof(u32);	
	reg_fps_12_index = sizeof(reg_fps_12_table)/sizeof(u32);
	reg_fps_15_index = sizeof(reg_fps_15_table)/sizeof(u32);    
	...
	memset(&reg_fps_7_table, 0, sizeof(reg_fps_7_table));  
	memset(&reg_fps_10_table, 0, sizeof(reg_fps_10_table)); 	
	memset(&reg_fps_12_table, 0, sizeof(reg_fps_12_table)); 
	memset(&reg_fps_15_table, 0, sizeof(reg_fps_15_table)); 
...
static int s5ka3dfx_set_resize(void)
{
	...
	dprintk(CAM_INF, S5KA3DFX_MOD_NAME "s5ka3dfx_set_resize is called...\n");

#if !(IS_USE_REGISTER_CONFIGURE_FILE_LSI)
	if(sensor->state == S5KA3DFX_STATE_PREVIEW) {
		switch(sensor->preview_size) {
		case S5KA3DFX_PREVIEW_SIZE_640_480:
			for (i = 0; i < reg_resize_vga_index; i++) {
				if(s5ka3dfx_set_data(client, reg_resize_vga_table[i]))
					goto resize_fail;
			}
			break;

		case S5KA3DFX_PREVIEW_SIZE_320_240:
			for (i = 0; i < reg_resize_qvga_index; i++) {
				if(s5ka3dfx_set_data(client, reg_resize_qvga_table[i]))
					goto resize_fail;
			}
			break;

		case S5KA3DFX_PREVIEW_SIZE_240_320:
			for (i = 0; i < reg_resize_ivs_qvga_index; i++) {
				if(s5ka3dfx_set_data(client, reg_resize_ivs_qvga_table[i]))
					goto resize_fail;
			}
			break;

		case S5KA3DFX_PREVIEW_SIZE_352_288:
			for (i = 0; i < reg_resize_cif_index; i++) {
				if(s5ka3dfx_set_data(client, reg_resize_cif_table[i]))
					goto resize_fail;
			}
			break;

		case S5KA3DFX_PREVIEW_SIZE_288_352:
			for (i = 0; i < reg_resize_ivs_cif_index; i++) {
				if(s5ka3dfx_set_data(client, reg_resize_ivs_cif_table[i]))
					goto resize_fail;
			}
			break;

		case S5KA3DFX_PREVIEW_SIZE_176_144:
			for (i = 0; i < reg_resize_qcif_index; i++) {
				if(s5ka3dfx_set_data(client, reg_resize_qcif_table[i]))
					goto resize_fail;
			}
			break;   

		case S5KA3DFX_PREVIEW_SIZE_144_176:
			for (i = 0; i < reg_resize_ivs_qcif_index; i++) {
				if(s5ka3dfx_set_data(client, reg_resize_ivs_qcif_table[i]))
					goto resize_fail;
			}
			break;
		...
		case S5KA3DFX_IMAGE_SIZE_640_480:
			for (i = 0; i < reg_resize_vga_index; i++) {
				if(s5ka3dfx_set_data(client, reg_resize_vga_table[i]))
					goto resize_fail;
			}
		...
		case S5KA3DFX_IMAGE_SIZE_320_240:
			for (i = 0; i < reg_resize_qvga_index; i++) {
				if(s5ka3dfx_set_data(client, reg_resize_qvga_table[i]))
					goto resize_fail;
			}
		...
		case S5KA3DFX_IMAGE_SIZE_240_320:
			for (i = 0; i < reg_resize_ivs_qvga_index; i++) {
				if(s5ka3dfx_set_data(client, reg_resize_ivs_qvga_table[i]))
					goto resize_fail;
			}
		...
		case S5KA3DFX_IMAGE_SIZE_352_288:
			for (i = 0; i < reg_resize_cif_index; i++) {
				if(s5ka3dfx_set_data(client, reg_resize_cif_table[i]))
					goto resize_fail;
			}
		...
		case S5KA3DFX_IMAGE_SIZE_288_352:
			for (i = 0; i < reg_resize_ivs_cif_index; i++) {
				if(s5ka3dfx_set_data(client, reg_resize_ivs_cif_table[i]))
					goto resize_fail;
			}
		...
		case S5KA3DFX_IMAGE_SIZE_176_144:
			for (i = 0; i < reg_resize_qcif_index; i++) {
				if(s5ka3dfx_set_data(client, reg_resize_qcif_table[i]))
					goto resize_fail;
			}
		...
		case S5KA3DFX_IMAGE_SIZE_144_176:
			for (i = 0; i < reg_resize_ivs_qcif_index; i++) {
				if(s5ka3dfx_set_data(client, reg_resize_ivs_qcif_table[i]))
					goto resize_fail;
			}
	...
	dprintk(CAM_INF, S5KA3DFX_MOD_NAME "s5ka3dfx_set_resize success...\n");

	return 0;

resize_fail:
	printk(S5KA3DFX_MOD_NAME "s5ka3dfx_set_resize is failed!!!\n");
	return -EINVAL;     
...
static u32 s5ka3dfx_insert_register_table(char* line)
{
	...
	else if ( parsing_section == REG_FPS_7_SECTION)             reg_fps_7_table[reg_fps_7_index++] = reg_val;  
	else if ( parsing_section == REG_FPS_10_SECTION)            reg_fps_10_table[reg_fps_10_index++] = reg_val;    
	else if ( parsing_section == REG_FPS_12_SECTION)            reg_fps_12_table[reg_fps_12_index++] = reg_val;  
	else if ( parsing_section == REG_FPS_15_SECTION)            reg_fps_15_table[reg_fps_15_index++] = reg_val;    
...
{
	if ( strstr(line, CAMIF_SET_SENSOR_VGA_INIT) != NULL )          parsing_section = REG_INIT_VGA_SECTION;    
	else if ( strstr(line, CAMIF_SET_SENSOR_VGA_VT_INIT) != NULL )       parsing_section = REG_INIT_VGA_VT_SECTION;     
	else if ( strstr(line, CAMIF_SET_SENSOR_WB_AUTO) != NULL )           parsing_section = REG_WB_AUTO_SECTION;
	else if ( strstr(line, CAMIF_SET_SENSOR_WB_DAYLIGHT ) != NULL )      parsing_section = REG_WB_DAYLIGHT_SECTION;
	else if ( strstr(line, CAMIF_SET_SENSOR_WB_CLOUDY ) != NULL )        parsing_section = REG_WB_CLOUDY_SECTION;
	else if ( strstr(line, CAMIF_SET_SENSOR_WB_INCANDESCENT) != NULL)    parsing_section = REG_WB_INCANDESCENT_SECTION;
	else if ( strstr(line, CAMIF_SET_SENSOR_WB_FLUORESCENT) != NULL)     parsing_section = REG_WB_FLUORESCENT_SECTION;
	else if ( strstr(line, CAMIF_SET_SENSOR_EV) != NULL )                parsing_section = REG_EV_SECTION;
	else if ( strstr(line, CAMIF_SET_SENSOR_EV_VT) != NULL )             parsing_section = REG_EV_VT_SECTION;    
	else if ( strstr(line, CAMIF_SET_SENSOR_EFFECT_NONE) != NULL )       parsing_section = REG_EFFECT_NONE_SECTION;
	else if ( strstr(line, CAMIF_SET_SENSOR_EFFECT_GRAY) != NULL )       parsing_section = REG_EFFECT_GRAY_SECTION;
	else if ( strstr(line, CAMIF_SET_SENSOR_EFFECT_RED) != NULL )        parsing_section = REG_EFFECT_RED_SECTION;    
	else if ( strstr(line, CAMIF_SET_SENSOR_EFFECT_SEPIA) != NULL )      parsing_section = REG_EFFECT_SEPIA_SECTION;
	else if ( strstr(line, CAMIF_SET_SENSOR_EFFECT_GREEN) != NULL )      parsing_section = REG_EFFECT_GREEN_SECTION;
	else if ( strstr(line, CAMIF_SET_SENSOR_EFFECT_AQUA) != NULL )       parsing_section = REG_EFFECT_AQUA_SECTION;
	else if ( strstr(line, CAMIF_SET_SENSOR_EFFECT_NEGATIVE) != NULL )   parsing_section = REG_EFFECT_NEGATIVE_SECTION;
	else if ( strstr(line, CAMIF_SET_SENSOR_FLIP_NONE) != NULL )         parsing_section = REG_FLIP_NONE_SECTION;    
	else if ( strstr(line, CAMIF_SET_SENSOR_FLIP_WATER) != NULL )        parsing_section = REG_FLIP_WATER_SECTION;    
	else if ( strstr(line, CAMIF_SET_SENSOR_FLIP_MIRROR) != NULL )       parsing_section = REG_FLIP_MIRROR_SECTION;    
	else if ( strstr(line, CAMIF_SET_SENSOR_FLIP_WATER_MIRROR) != NULL ) parsing_section = REG_FLIP_WATER_MIRROR_SECTION;    
	else if ( strstr(line, CAMIF_SET_SENSOR_PRETTY_NONE) != NULL )       parsing_section = REG_PRETTY_NONE_SECTION;    
	else if ( strstr(line, CAMIF_SET_SENSOR_PRETTY_LEVEL1) != NULL )     parsing_section = REG_PRETTY_LEVEL1_SECTION;    
	else if ( strstr(line, CAMIF_SET_SENSOR_PRETTY_LEVEL2) != NULL )     parsing_section = REG_PRETTY_LEVEL2_SECTION;    
	else if ( strstr(line, CAMIF_SET_SENSOR_PRETTY_LEVEL3) != NULL )     parsing_section = REG_PRETTY_LEVEL3_SECTION;        
	else if ( strstr(line, CAMIF_SET_SENSOR_PRETTY_VT_NONE) != NULL )    parsing_section = REG_PRETTY_VT_NONE_SECTION;    
	else if ( strstr(line, CAMIF_SET_SENSOR_PRETTY_VT_LEVEL1) != NULL )  parsing_section = REG_PRETTY_VT_LEVEL1_SECTION;    
	else if ( strstr(line, CAMIF_SET_SENSOR_PRETTY_VT_LEVEL2) != NULL )  parsing_section = REG_PRETTY_VT_LEVEL2_SECTION;    
	else if ( strstr(line, CAMIF_SET_SENSOR_PRETTY_VT_LEVEL3) != NULL )  parsing_section = REG_PRETTY_VT_LEVEL3_SECTION;     
	else if ( strstr(line, CAMIF_SET_SENSOR_FPS7) != NULL )              parsing_section = REG_FPS_7_SECTION;    
	else if ( strstr(line, CAMIF_SET_SENSOR_FPS10) != NULL )             parsing_section = REG_FPS_10_SECTION; 
	else if ( strstr(line, CAMIF_SET_SENSOR_FPS12) != NULL )             parsing_section = REG_FPS_12_SECTION;    
	else if ( strstr(line, CAMIF_SET_SENSOR_FPS15) != NULL )             parsing_section = REG_FPS_15_SECTION;   
	else return -EINVAL;
...
static int ioctl_s_parm(struct v4l2_int_device *s, struct v4l2_streamparm *a)
{
	...
	if(sensor->mode < 1 || sensor->mode > 3) 
