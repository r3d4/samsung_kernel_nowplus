...
static int ce147_write_read_reg(struct i2c_client *client, u8 writedata_num, const u8* writedata, u8 readdata_num, u8* readdata)
{
  ...
  if (!client->adapter) 
  {
    printk(CE147_MOD_NAME "can't search i2c client adapter\n");
    return -ENODEV;
  }
  ...
  for(i = 0; i < 5; i++)
  {
    ...
    else
    {
      ...
      {
        printk(CE147_MOD_NAME "ce147_write_read_reg is failed and power off\n");
...
static int ce147_write_reg(struct i2c_client *client, u8 length, const u8* readdata)
{
  ...
  for (i = 0; i < 5; i++)
  {
    ...
    else
    {
      if((i == 4) || (temp == -110 || temp == -5))
      {
        printk(CE147_MOD_NAME "ce147_write_reg is failed and power off\n");
        sensor->pdata->power_set(V4L2_POWER_OFF);
        return temp;
      }
    }
...
static int ce147_poll_reg(struct i2c_client *client, enum ReadCommad CheckStatus)
{
  ...
  switch(CheckStatus)
  {
    case RCommandPreviewStatusCheck:
      ...
      MaxRetryCount = 1000; // Modified retrycount to fix ECAM no data error
      PollingDelay  = 5;
      break;
  ...
  {   
    if(CheckStatus == RCommandDZoomFinishStatus)
    {
      ...
      if((ret == -EINVAL) || (ret == -110) || (ret == -5))
      {
        printk(CE147_MOD_NAME " Poll_Reg : i2c Write Failed! out right \n");
        break;
      }
    }
    else
...
#ifdef CONFIG_MACH_OSCAR ///// PROJECT DIVISTION ///////////
  mdelay(700);
#else
  mdelay(600);
#endif
...
#if defined (CONFIG_MACH_OSCAR)
#if (CONFIG_OSCAR_REV <= CONFIG_OSCAR_REV01)
  sensor->flash_capture = CE147_FLASH_CAPTURE_OFF;
  switch(sensor->flash_capture)
  {
    case CE147_FLASH_CAPTURE_OFF:
      ce147_write_reg(client, sizeof(FlashAFOff_list), FlashAFOff_list);
      ce147_write_reg(client, sizeof(FlashCaptureOff_list), FlashCaptureOff_list);
      break;
    case CE147_FLASH_CAPTURE_ON:
      ce147_write_reg(client, sizeof(FlashAFOn_list), FlashAFOn_list);
      ce147_write_reg(client, sizeof(FlashCaptureOn_list), FlashCaptureOn_list);
      break;
    case CE147_FLASH_CAPTURE_AUTO:
      ce147_write_reg(client, sizeof(FlashAFAuto_list), FlashAFAuto_list);
      ce147_write_reg(client, sizeof(FlashCaptureAuto_list), FlashCaptureAuto_list);
      break;
    default:
    ...
    case CE147_AF_START :
      ...
      if(sensor->focus_mode == CE147_AF_INIT_FACE)
      {
        if(ce147_write_reg(client, sizeof(FaceDetection_UnLock_list), FaceDetection_UnLock_list))
          goto focus_status_fail;
        if(ce147_write_reg(client, sizeof(FaceDetection_box_Off_list), FaceDetection_box_Off_list))
          goto focus_status_fail;
      }
      else
      {
...
static int ce147_set_preview(void)
{
  ...
  /* Fliker 60Hz */
  if(ce147_write_reg(client, sizeof(ConfigSensorfliker_list), ConfigSensorfliker_list))
    goto preview_fail;
  ...
  if(ce147_720p_enable)
  {
    ...
    if(ce147_write_reg(client, sizeof(ISOCamcorderHD_list), ISOCamcorderHD_list))
      goto preview_fail;
#if (ISP_SUPPORT_COMPANY == SAMSUNG_FIBER_OPTICS)
    if(ce147_write_reg(client, sizeof(AE_Speed_Ctrl_2sec), AE_Speed_Ctrl_2sec))
      goto preview_fail;
...
static void ce147_set_skip(void)
{
  ...
  { 
    if(ce147_curr_state == CE147_STATE_INVALID)
    {
...
static int ioctl_streamon(struct v4l2_int_device *s)
{
  ...
  if(sensor->state != CE147_STATE_CAPTURE)
  {
    ...
    {
      /* Lens focus setting */
      if(ce147_set_focus(CE147_AF_INIT_NORMAL))
        goto streamon_fail;       
    }
...
static int ioctl_g_ctrl(struct v4l2_int_device *s, struct v4l2_control *vc)
{
  ...
  dprintk(CAM_INF, CE147_MOD_NAME "ioctl_g_ctrl is called...id(%d)\n", vc->id);
...
static int ioctl_enum_framesizes(struct v4l2_int_device *s, struct v4l2_frmsizeenum *frms)
{
  ...
  {    
    ...
    if(sensor->capture_size == ARRAY_SIZE(ce147_image_sizes))
...
static int ce147_probe(struct i2c_client *client, const struct i2c_device_id *device)
{
  ...
  dprintk(CAM_INF, CE147_MOD_NAME "ce147_probe is called...\n");
