...
static int ce147_write_read_reg(struct i2c_client *client, u8 writedata_num, 
	const u8* writedata, u8 readdata_num, u8* readdata)
{
...
static int ce147_detect(struct i2c_client *client)
{
	...
	dprintk(CAM_DBG " Loader (FW ver)  : 0x%02x, 0x%02x\n", ISP_FW_ver[0], ISP_FW_ver[1]);
	dprintk(CAM_DBG " Loader (PRM ver) : 0x%02x, 0x%02x\n", ISP_FW_ver[2], ISP_FW_ver[3]);
	dprintk(CAM_DBG " Sensor is Detected!!!\n");
...
static int ioctl_s_fmt_cap(struct v4l2_int_device *s, struct v4l2_format *f)
{
	...
	dprintk(CAM_DBG "camera mode  : %d (1:camera , 2:camcorder, 3:vt)\n", sensor->mode);
	dprintk(CAM_DBG "camera state : %d (0:preview, 1:snapshot)\n", sensor->state);
	dprintk(CAM_DBG "set width  : %d\n", pix->width);
	dprintk(CAM_DBG "set height : %d\n", pix->height); 
