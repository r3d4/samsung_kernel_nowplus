...
static unsigned cm_fclken_iva2;
...
#ifdef CONFIG_BRIDGE_DVFS
		clk_handle = clk_get(NULL, "dsp_fck");
		if (!clk_handle)
...
static int BRIDGE_SUSPEND(struct platform_device *pdev, pm_message_t state)
{
	...
	/* FCLK in CM_FCLKEN_IVA2 set to off */
	cm_fclken_iva2 = omap_readl(0x48004000);
	omap_writel(0,0x48004000);
	cm_fclken_iva2 &= 0x1;
...
static int BRIDGE_RESUME(struct platform_device *pdev)
{
	...
	/* FCLK in CM_FCLKEN_IVA2 restored */
	omap_writel(cm_fclken_iva2, 0x48004000);
