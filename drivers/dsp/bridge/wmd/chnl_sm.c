			       ...
			       OPTIONAL u32 dw_dsp_addr, u32 dw_arg)
{
	...
	/* Gerrit Patch Ib95939bf:  DSP changes to let it run with CONFIG_PM turned on*/
	 if (dev_ctxt->dw_brd_state == BRD_DSP_HIBERNATION) {
	 /*   
         * The bridge resume code doesn't wake the DSP if it put itself
	 * into hibernation (as opposed to the MPU putting it into
	 * hibernation).  So we need to wake it up now.
	 */

		 wake_dsp(dev_ctxt, NULL);

		 if (dev_ctxt->dw_brd_state != BRD_RUNNING) {
			 status = -EFAULT;
			 goto func_end;
			 }    
	 }    
...
func_cont:
	...
	/*Gerrit patch I0dd9fffd: DSPBRIDGE: PM: Avoid mbox bus error after hibernation */
	if (dev_ctxt->dw_brd_state == BRD_DSP_HIBERNATION || dev_ctxt->dw_brd_state == BRD_HIBERNATION)
       	{
