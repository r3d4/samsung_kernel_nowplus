...
#include "pm.h"
#define CONFIG_OMAP3_MPU_L2_CACHE_WORKAROUND
#define TEMPORARY_FIX_FOR_SGX_BLOCKING_RETENTION
#ifndef TEMPORARY_FIX_FOR_SGX_BLOCKING_RETENTION
#include <linux/io.h>
#include <linux/clk.h>
...
#ifdef DEBUG_CPU_RETENTION
	int mpu_prev, core_prev;
#endif
#ifndef TEMPORARY_FIX_FOR_SGX_BLOCKING_RETENTION
	struct powerdomain *dss_pd;
	static int enter_idle = 0 ;
	struct clk *clk;
	int ret_clk = 0;
	u32 regval;
#ifndef DEBUG_CPU_RETENTION
	int mpu_prev, core_prev;
#endif

	dss_pd = pwrdm_lookup("dss_pwrdm");
	clk = clk_get(NULL, "dss_ick");
#endif
...
/*	else{
...
#ifndef TEMPORARY_FIX_FOR_SGX_BLOCKING_RETENTION
	mpu_prev = pwrdm_read_prev_pwrst(mpu_pd);
	// core_prev = pwrdm_read_prev_pwrst(core_pd);
	// if(((mpu_prev == 0) && (core_prev == 1))&&(enter_idle==0)){
	//if(((mpu_prev <= 1) && (core_next == 1))&&(dss_next >= 1) && (enter_idle==0)){
	if ((cx->type == 5) && (!stop_vsync_disable))
	{
		regval =0 ;
		regval = omap_readl(0x48004D00);
		regval= ((regval>>29) & (0x1));
		...
		if(!regval)
		{
			...
			//Autoidle        
			regval = omap_readl(0x48004e30);
			regval &= ~0x01;
			omap_writel(regval,0x48004E30);

			ret_clk = clk_enable(clk);

			//printk("clk enbled o ret = %d   \n",ret_clk);
			//disable SYSCLOST and VSYNC interrupts together(DISPC_IRQENABLE &= 0x4002)
			regval = omap_readl(0x4805041C);
			regval &=  ~0x4002;
			omap_writel(regval,0x4805041C);

			// acknowledge SYSCLOST and VSYNC interrupts(DISPC_IRQSTATUS |= 0x4002) 
			regval = omap_readl(0x48050418);
			regval |=  0x4002;
			omap_writel(regval,0x48050418);
	...
	mpu_prev = pwrdm_read_prev_pwrst(mpu_pd);
	core_prev = pwrdm_read_prev_pwrst(core_pd);
	// if(((mpu_prev == 0) && (core_prev == 1))&& (enter_idle==1)){
	if (enter_idle)
	{
		enter_idle = 0;
		//AUTOIDLE
		...
		//printk("clk enbled  e2\n");
		// reenable SYSCLOST and VSYNC interrupts together(DISPC_IRQENABLE &= 0x4002)

		regval = omap_readl(0x4805041C);
		regval |=  0x4002;
		omap_writel(regval,0x4805041C);

		//printk("clk disenbled  e\n");
...
return_sleep_time:
	...
	mpu_prev = pwrdm_read_prev_pwrst(mpu_pd);
	core_prev = pwrdm_read_prev_pwrst(core_pd);
	if ((mpu_prev == 0) && (core_prev == 1)) {
		 printk("[CPU] in state %d -- %d us\n", cx->type, (u32)timespec_to_ns(&ts_idle)/1000);
	 }
#endif
	local_irq_enable();
	local_fiq_enable();
