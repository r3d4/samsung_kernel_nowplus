...
static void omap_uart_save_context(struct omap_uart_state *uart)
{
    ...
    uart->efr=serial_read_reg(uart, UART_EFR);
...
static void omap_uart_restore_context(struct omap_uart_state *uart)
{
	...
	/* Config B mode */
	serial_write_reg(uart, UART_LCR, OMAP_UART_LCR_CONF_MDB);
	serial_write_reg(uart, UART_EFR, uart->efr);
...
void omap_uart_prepare_suspend(void)
{
	...
	list_for_each_entry(uart, &uart_list, node) {
  /* Save context isa added to solve BT problem timeout after suspend
    removed save context from disable clock*/
