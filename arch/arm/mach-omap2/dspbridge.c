...
#ifdef CONFIG_BRIDGE_DVFS
static void dsp_set_min_opp(struct device *dev, unsigned long f)
{

        struct device *dsp_dev;
        struct omap_opp *opp;
        unsigned long rate = f;

        dsp_dev = omap2_get_iva_device();

        opp = opp_find_freq_ceil(dsp_dev, &rate);

        omap_device_set_rate(dsp_dev, dsp_dev, rate);

        printk(KERN_ERR "[BRIDGE DVFS] Minimum opp is set to %u (%u)\n", rate, f);

        return;

}

static unsigned char dsp_get_opp(void)
{
        struct device *dsp_dev;
        struct omap_opp *opp;
        unsigned long rate;
        unsigned char opp_id;

        dsp_dev = omap2_get_iva_device();

        rate = opp_get_rate(dsp_dev);

        opp = opp_find_freq_ceil(dsp_dev, &rate);

        opp_id = opp_get_opp_id(opp);

        printk(KERN_ERR "[BRIDGE DVFS] Current opp id: %d\n", opp_id);

        return opp_id;

}

static void dsp_set_freq(unsigned long f)
{
        struct device *dsp_dev;

        dsp_dev = omap2_get_iva_device();

        opp_set_rate(dsp_dev, f);

        printk(KERN_ERR "[BRIDGE DVFS] DSP Current freq set to %u\n", f);

}

static unsigned long dsp_get_freq(void)
{
        struct device *dsp_dev;
        unsigned long rate;
        ...
        printk(KERN_ERR "[BRIDGE DVFS] DSP Current freq is %u\n", rate);

        return rate;

}

struct omap_opp *get_dsp_rate_table(void)
{
        return NULL;
}
#endif
...
#ifdef CONFIG_BRIDGE_DVFS
	.dsp_set_min_opp = dsp_set_min_opp,
	.dsp_get_opp	 = dsp_get_opp,
	.cpu_set_freq	 = dsp_set_freq,
	.cpu_get_freq	 = dsp_get_freq,
	.dsp_get_rate_table = get_dsp_rate_table,
#endif
...
static int get_opp_table(struct dspbridge_platform_data *pdata)
{
...
#if 1
        dsp_dev = omap2_get_iva_device();
	opp_count = opp_get_opp_count(dsp_dev);

#if 0
	dsp_rate = (*pdata->dsp_get_rate_table)();
#endif

	dsp_sort = kzalloc(sizeof(struct dsp_rate) * (opp_count + 1),
			   GFP_KERNEL);
	if (!dsp_sort) {
	...
	for (i = 1; i <= opp_count; i++) {
                dsp_rate = opp_find_freq_ceil(dsp_dev, &old_rate);
                dsp_sort[i].rate = opp_get_freq(dsp_rate);
                dsp_sort[i].u_volts = opp_get_voltage(dsp_rate);
                ++old_rate;
        }
	...
	for (i = 1; i <= opp_count; i++) {
		...
		if (rate == old_rate) {
			...
			pdata->dsp_freq_table[i].u_volts = dsp_sort[i].u_volts;
			/*
			 * min threshold:
