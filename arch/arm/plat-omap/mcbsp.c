...
static void omap_TI_WA(unsigned int id)
{
...
struct omap_mcbsp *mcbsp;
	...
	mcbsp = id_to_mcbsp_ptr(id);	
		...
		u32  CM_ICLKEN_PER = 0 ;
		u32 CM_IDLEST_PER= 0 ;
		u32 CONTROL_DEVCONF0 = 0;
		u32 MCBSPLP_SYSCONFIG_REG = 0;
		u32 Smart_idle = 0;

CM_FCLKEN_PER                = omap_readl(0x48005000);
CM_ICLKEN_PER                 = omap_readl(0x48005010);
CM_IDLEST_PER                 = omap_readl(0x48005020);

CM_FCLKEN_PER = CM_FCLKEN_PER & 0x1;
CM_ICLKEN_PER = CM_ICLKEN_PER & 0x1;
CM_IDLEST_PER = CM_IDLEST_PER & 0x01;

if((CM_FCLKEN_PER !=1)||(CM_ICLKEN_PER !=1 )||(CM_IDLEST_PER != 0))
{
printk("******REPORT THIS TO TI  OMAPS00223636*********\n");
if((CM_ICLKEN_PER !=1)||(CM_IDLEST_PER != 0))
{
printk("Enabling Interface clocks !!!\n");
omap_writel((omap_readl(0x48005030)&~(0x01)), 0x48005030);//disable autioidle of McBSP2 temporarily
idle_set = 1;
clk_enable(mcbsp->iclk);
}
if((CM_FCLKEN_PER !=1))
{
printk("Enabling Functional clocks !!!\n");
clk_enable(mcbsp->fclk);
}
CM_IDLEST_PER                  = omap_readl(0x48005020);
CONTROL_DEVCONF0               = omap_readl(0x48002274);
MCBSPLP_SYSCONFIG_REG          = omap_readl(0x4902208C);
CM_FCLKEN_PER                  = omap_readl(0x48005000);
CM_ICLKEN_PER                  = omap_readl(0x48005010);
printk("CM_FCLKEN_PER  %x  \n",CM_FCLKEN_PER);
printk("CM_ICLKEN_PER  %x  \n",CM_ICLKEN_PER);
printk("CM_IDLEST_PER  %x  \n",CM_IDLEST_PER);
printk("CONTROL_DEVCONF0    %x \n",CONTROL_DEVCONF0);
printk("MCBSPLP_SYSCONFIG_REG  %x \n",MCBSPLP_SYSCONFIG_REG);
printk("CM_AUTOIDLE_PER  %x  \n",omap_readl(0x48005030));
/*
Smart_idle = ((MCBSPLP_SYSCONFIG_REG & 0x18)>>3);
if(Smart_idle != 2 )
{
printk("Forcing Smart Idle if not in Smart idle mode !!!\n");
MCBSPLP_SYSCONFIG_REG = MCBSPLP_SYSCONFIG_REG & 0xFFFFFFE7;
MCBSPLP_SYSCONFIG_REG = MCBSPLP_SYSCONFIG_REG |0x10 ;
omap_writel(MCBSPLP_SYSCONFIG_REG,0x4902208C);
...
static void omap_TI_WA_after()
{	
if(idle_set )
{
printk("TI recovery code omap_TI_WA_after\n");
idle_set = 0;
omap_writel(omap_readl(0x48005030)|0x01, 0x48005030);//restore autioidle of McBSP3 again
}
}
...
static irqreturn_t omap_mcbsp_tx_irq_handler(int irq, void *dev_id)
{
	...
	u16 irqst_spcr2;
#if 0//TI HS.Yoon 20100930 DMB noise patch
	u16 irq_status;
#endif
	irqst_spcr2 = OMAP_MCBSP_READ(mcbsp_tx->io_base, SPCR2);
#if 0//TI HS.Yoon 20100930 DMB noise patch
	irq_status = OMAP_MCBSP_READ(mcbsp_tx->io_base, IRQST);	
#endif
...
#if 0//TI HS.Yoon 20100930 DMB noise patch	
	else if(irq_status & XOVFLSTAT) {
            dev_err(mcbsp_tx->dev,"TX Overflow error! : 0x%x\n",
                    irq_status);
            // Writing one to XOVFLSTAT clears the IRQ 
            OMAP_MCBSP_WRITE(mcbsp_tx->io_base, IRQST,
                    irq_status & (XOVFLSTAT));
     }	
#endif
	else {
...
#if 0//TI HS.Yoon 20100930 DMB noise patch
	irqen=OMAP_MCBSP_READ(io_base, IRQEN);
        // Writing one to XOVFLSTAT enable the IRQ
        OMAP_MCBSP_WRITE(io_base, IRQEN, irqen | (XOVFLSTAT));
#endif
