...
#if 1 
/*
 * Following is a Froyo pad config which has erros interms of offsets
 */
...
static int omap_gpio_suspend(struct device *dev)
{
...
#ifdef __FIX_GPIO_OUTPUT_GLITCH_IN_OFF_MODE__
	{
		int j;
		for (j = 0; j < 32; j++) {
			int offset = gpio_pad_map[j + pdev->id * 32];
			u16 v;
...
#ifdef __FIX_GPIO_OUTPUT_GLITCH_IN_OFF_MODE__
void omap3_gpio_save_pad_context(void)
{
	struct gpio_bank *bank;
	struct gpio_pad *pad;
	int i;
...
void omap3_gpio_restore_pad_context(void)
{
	struct gpio_pad *pad;
	int i;
