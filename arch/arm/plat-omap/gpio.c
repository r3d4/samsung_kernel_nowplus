...
#define __GPIO_GLITCH_WA__
#ifdef __GPIO_GLITCH_WA__ //TI HS.Yoon 20100105
static u16 gpio_pads_4glitch[OMAP34XX_GPIO_AMT];
#endif
...
static void _set_gpio_dataout(struct gpio_bank *bank, int gpio, int enable)
{
	...
	case METHOD_GPIO_24XX:
#ifdef __GPIO_GLITCH_WA__ //TI HS.Yoon 20100105
		if (gpio_pads_4glitch == NULL)
		{
			//gpio_pads isn't initialized before use
			//Insert your error message!!!
			return;
		}
		//printk(" # start #\n\n");
		//printk("[_set_gpio_dataout] reg = %08x",  reg);
		if (enable)
		{
			if(reg == OMAP34XX_GPIO2_BASE)
			{
				//printk("[_set_gpio_dataout] Set gpio #= %d, offset:%x\n",  gpio+32, gpio_pads_4glitch[gpio]);
				omap_ctrl_writew(0x011C, gpio_pads_4glitch[gpio]);
			}
			else if(reg == OMAP34XX_GPIO3_BASE)
			{
				//printk("[_set_gpio_dataout] Set gpio #= %d, offset:%x\n",  gpio+64, gpio_pads_4glitch[gpio+32]);
				omap_ctrl_writew(0x011C, gpio_pads_4glitch[gpio+32]);
			}
			else if(reg == OMAP34XX_GPIO4_BASE)
			{
				//printk("[_set_gpio_dataout] Set gpio #= %d, offset:%x\n",  gpio+96, gpio_pads_4glitch[gpio+64]);
				omap_ctrl_writew(0x011C, gpio_pads_4glitch[gpio+64]);
			}
			else if(reg == OMAP34XX_GPIO5_BASE)
			{
				//printk("[_set_gpio_dataout] Set gpio #= %d, offset:%x\n",  gpio+128, gpio_pads_4glitch[gpio+96]);
				omap_ctrl_writew(0x011C, gpio_pads_4glitch[gpio+96]);
			}
			else if(reg == OMAP34XX_GPIO6_BASE)
			{
				//printk("[_set_gpio_dataout] Set gpio #= %d, offset:%x\n",  gpio+160, gpio_pads_4glitch[gpio+128]);
				omap_ctrl_writew(0x011C, gpio_pads_4glitch[gpio+128]);
			}
		...
		else
		{
			if(reg == OMAP34XX_GPIO2_BASE)
			{
				//printk("[_set_gpio_dataout] Clear gpio #= %d, offset:%x\n",  gpio+32, gpio_pads_4glitch[gpio]);
				omap_ctrl_writew(0x010C, gpio_pads_4glitch[gpio]);
			}
			else if(reg == OMAP34XX_GPIO3_BASE)
			{
				//printk("[_set_gpio_dataout] Clear gpio #= %d, offset:%x\n",  gpio+64, gpio_pads_4glitch[gpio+32]);
				omap_ctrl_writew(0x010C, gpio_pads_4glitch[gpio+32]);
			}
			else if(reg == OMAP34XX_GPIO4_BASE)
			{
				//printk("[_set_gpio_dataout] Clear gpio #= %d, offset:%x\n",  gpio+96, gpio_pads_4glitch[gpio+64]);
				omap_ctrl_writew(0x010C, gpio_pads_4glitch[gpio+64]);
			}
			else if(reg == OMAP34XX_GPIO5_BASE)
			{
				//printk("[_set_gpio_dataout] Clear gpio #= %d, offset:%x\n",  gpio+128, gpio_pads_4glitch[gpio+96]);
				omap_ctrl_writew(0x010C, gpio_pads_4glitch[gpio+96]);
			}
			else if(reg == OMAP34XX_GPIO6_BASE)
			{
				//printk("[_set_gpio_dataout] Clear gpio #= %d, offset:%x\n",  gpio+160, gpio_pads_4glitch[gpio+128]);
				omap_ctrl_writew(0x010C, gpio_pads_4glitch[gpio+128]);
			}
...
#endif /* defined(CONFIG_ARCH_OMAP3) */
#endif /* defined(CONFIG_ARCH_OMAP3) || defined(CONFIG_ARCH_OMAP4) */
...
#if 1 // FIXME, fixed enable_irq_wake failure
			if (cpu_is_omap24xx() &&
