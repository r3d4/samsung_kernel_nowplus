	...
	const struct snd_kcontrol_new *controls, int num_controls)
{
	...
	for (i = 0; i < num_controls; i++) {
		...
		#ifdef SOCCORE_DEBUG
		printk("SOC-core: addcontrol codec= %s control=%s", codec->name,control->name);
		#endif
	}
...
{
     printk("coresoc:%s, clk=%d, freq=%d, dir=%d\n", __func__, clk_id, freq, dir);
