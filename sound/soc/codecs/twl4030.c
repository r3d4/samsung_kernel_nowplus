...
static const char *audio_path[]     = {"Playback Path", "Voice Call Path", "Memo Path", "VOIP Call Path","Idle Mode","Mic Mute","VR Mode"};
...
twl4030_codec_setting voipcall_rcv[]={
  ...
  //{0x21, 0x34},  //TWL4030_REG_EAR_CTL
  {0x3a, 0x16},  //TWL4030_REG_APLL_CTL
  {0x25, 0x10},  //TWL4030_REG_PREDL_CTL
  {0x26, 0x10},  //TWL4030_REG_PREDR_CTL
  {0x0e, 0x01},  //TWL4030_REG_AUDIO_IF
  {0x17, 0x08},  //TWL4030_REG_AVDAC_CTL
...
twl4030_codec_setting voipcall_hp3p[]= {
  ...
  {0x13, 0x7f},  //TWL4030_REG_ARXL2PGA
  {0x17, 0x0c},  //TWL4030_REG_AVDAC_CTL
  {0x1b, 0x03},  //TWL4030_REG_ARXL2_APGA_CTL
  {0x1c, 0x03},  //TWL4030_REG_ARXR2_APGA_CTL
  {0x22, 0x24},  //TWL4030_REG_HS_SEL
  {0x23, 0x09},  //TWL4030_REG_HS_GAIN_SET
  {0x24, 0x41},  //TWL4030_REG_HS_POPN_SET
  {0x24, 0x42},  //TWL4030_REG_HS_POPN_SET
  {0x3a, 0x16},  //TWL4030_REG_APLL_CTL
...
{
	u8 data = 0x00;
	P("IDLE mode - Power resources has to be set \n");
	...
	switch(ucontrol->value.integer.value[0])
	{
		...
		case MAIN_MIC:
	        ...
	        else if(twl4030_voip_device == RCV)
	        {
	            ...
	            twl4030_write(codec, 0x25, 0x10);
	            twl4030_write(codec, 0x26, 0x10);
	        }
	        ...
	        else if(twl4030_voip_device == HP3P)
	        {
	            twl4030_write(codec, 0x0c, 0x08);
	            twl4030_write(codec, 0x48, 0x24);
	            twl4030_write(codec, 0x1b, 0x03);
	            twl4030_write(codec, 0x1c, 0x03);
...
static int twl4030_soc_resume(struct snd_soc_codec *codec)
{
	...
	if((twl4030_mode !=VOICE_CALL)&&(twl4030_mode !=VOICE_MEMO)&&(twl4030_mode !=VOIP_CALL))
	{
		 ...
		 twl4030_set_bias_level(codec, codec->dapm->suspend_bias_level);
	}
