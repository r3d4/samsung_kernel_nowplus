...
#define ZEUS_PCM_SELECT_GPIO	52
	...
	struct snd_pcm_hw_params *params)
{
...
#if 1	/* Set codec DAI configuration */
...
#else
		ret = snd_soc_dai_set_fmt(codec_dai,
 				  SND_SOC_DAIFMT_I2S_1PHASE |  //Swap Fix:Changing this into single phase
				  SND_SOC_DAIFMT_NB_NF |
				  SND_SOC_DAIFMT_CBM_CFM);
#endif
	...
	ret = snd_soc_dai_set_fmt(cpu_dai,
				  SND_SOC_DAIFMT_I2S |  //Swap Fix:Changing this into single phase in McBSP Solves dynamic swaping.    
				  SND_SOC_DAIFMT_NB_NF |
				  SND_SOC_DAIFMT_CBM_CFM);
	if (ret < 0) {
...
static int sdp3430_twl4030_init(struct snd_soc_codec *codec)
{
	...
	omap_cfg_reg(H3_34XX_GPIO52_OUT);
	...
	if (ret)
		return ret;
         ...
         /* add MAX9877 specific controls */
	ret =max9877_add_controls(codec);
	if (ret)
	{
		printk( "********** max9877_add_controls , %d\n", ret);
	}
	...
	snd_soc_dapm_enable_pin(codec, "EARPIECE"); // reciever
...
//	snd_soc_dapm_nc_pin(codec, "PREDRIVER");
	...
	snd_soc_dapm_nc_pin(codec, "Ext Spk");
	...
	if (gpio_request(ZEUS_PCM_SELECT_GPIO, "Mcbsp_mux") == 0) 
	{
		gpio_direction_output(ZEUS_PCM_SELECT_GPIO, 0);
	//	printk( "********** ZEUS_PCM_SELECT_GPIO :  set low\n");
	}
...
static int __init sdp3430_soc_init(void)
{
	...
	u8 pin_mux;
	        ...
	        printk(KERN_EMERG " %s : %s : %i ---SSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSS\n", __FILE__, __FUNCTION__, __LINE__);
	...
	*(unsigned int *)sdp3430_dai[1].cpu_dai->private_data = 2; /* McBSP3 */
	...
	if (ret)
		goto err1;
	      ...
	      printk(KERN_EMERG "RRRRRRRRRRRRRRRRRRRRRRRRRRRRRR---------------SSSSSSSSSSSSSSSSSSSSSSSSSSSSSS\n");	
