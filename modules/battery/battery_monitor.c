...
#include <mach/omap3-gptimer12.h>
#include "common.h"
				    ...
				    const char *buf, size_t size)
{
	...
	printk("[BR] boot complete flag:%d, buf:%s, size:%d\n",flag, buf, size);
...
int turn_resources_on_for_adc()
{
	...
	ret = twl4030_i2c_read_u8( TWL4030_MODULE_MADC, &val, TWL4030_MADC_CTRL1 );
	val &= ~TWL4030_MADC_MADCON;
	ret = twl4030_i2c_write_u8( TWL4030_MODULE_MADC, val, TWL4030_MADC_CTRL1 );
	msleep( 10 );
	ret = twl4030_i2c_read_u8( TWL4030_MODULE_MADC, &val, TWL4030_MADC_CTRL1 );
	val |= TWL4030_MADC_MADCON;
	ret = twl4030_i2c_write_u8( TWL4030_MODULE_MADC, val, TWL4030_MADC_CTRL1 );
	...
	twl4030_i2c_read_u8( TWL4030_MODULE_USB, &val, 0xFE/*PHY_CLK_CTRL*/ );
	val |= 0x1;
	twl4030_i2c_write_u8( TWL4030_MODULE_USB, val, 0xFE/*PHY_CLK_CTRL*/ );

	twl4030_i2c_write_u8( TWL4030_MODULE_PM_RECEIVER, VSEL_VINTANA2_2V75, TWL4030_VINTANA2_DEDICATED );    
	twl4030_i2c_write_u8( TWL4030_MODULE_PM_RECEIVER, 0x20, TWL4030_VINTANA2_DEV_GRP );
#endif
	return 0;
}
...
//				do_fuelgauge_reset();
	...
	{
		printk("[BR] boot complete \n");
		boot_complete = true;
	}
#endif

#if 1 
	printk( "[BR] monitor BATT.(%d%%, %dmV, %d*, count=%d)\n", 
...
#else
	queue_delayed_work( sec_bci.sec_battery_workq, &di->battery_monitor_work, HZ );
#endif	/* CONFIG_SEC_BATTERY_USE_RECOVERY_MODE */
