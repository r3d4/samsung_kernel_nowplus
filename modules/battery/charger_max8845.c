...
int _cable_status_now_( void )
{
	...
	{
		ret = get_real_usbic_state(); //get_usbic_state(); 
	}
...
#ifdef CONFIG_MACH_HALO  // check battery capacity (1800mAh)
	if ( device_config->VF_CHECK_USING_ADC )
	{
		//msleep( 500 );
		count = _get_t2adc_data_( device_config->VF_ADC_PORT );
		printk("HALO Battery vf : %d\n", count);
		if(count < 120 || count > 210)
			return false;
	}
#endif
...
// Return Value   : None
{
	...
	n_usbic_state = get_real_usbic_state(); // get_usbic_state();
	...
	case MICROUSBIC_USB_CABLE :
		...
		twl4030_i2c_write_u8( TWL4030_MODULE_PM_RECEIVER, REMAP_ACTIVE, TWL4030_VUSB3V1_REMAP );
		twl4030_i2c_write_u8( TWL4030_MODULE_PM_RECEIVER, REMAP_ACTIVE, TWL4030_VINTANA2_REMAP );
	...
	default:
		...
		twl4030_i2c_write_u8( TWL4030_MODULE_PM_RECEIVER, REMAP_OFF, TWL4030_VUSB3V1_REMAP );
...
// Return Value   : None
{
	...
	while ( count < 10 )
	{
		...
		if(sec_bci->battery.battery_level_vol >= 4200){
			break;
		}
#endif
		if ( !gpio_get_value( KCHG_ING_GPIO ) )
		{
			if(sec_bci->battery.battery_health == POWER_SUPPLY_HEALTH_DEAD)
			{
				printk("%s, CHG_ING is low, Battery health is dead. - recharging\n", __func__);
				sec_bci->battery.battery_vf_ok = check_battery_vf();
				change_charge_status( POWER_SUPPLY_STATUS_CHARGING, CHARGE_DUR_ACTIVE );
			}
			printk("%s, CHG_ING is low, count:%d\n", __func__, count);
			goto Enable_IRQ_Full_Det;
			break;
		}
		msleep( 10 );
