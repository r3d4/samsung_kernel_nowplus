...
static int dpram_read(dpram_device_t *device, const u16 non_cmd)
{
	...
	if (head != tail) {
		...
		if (head > tail) {
			size = head - tail;
			tty_insert_flip_string(device->serial.tty, (unsigned char *)(DPRAM_VBASE + (device->in_buff_addr + tail)), size);
			...
			tty_insert_flip_string(device->serial.tty, (unsigned char *)(DPRAM_VBASE + (device->in_buff_addr + tail)), tmp_size);
			...
			if (size > tmp_size) {
				tty_insert_flip_string(device->serial.tty, (unsigned char *)(DPRAM_VBASE + device->in_buff_addr), size - tmp_size);
		...
		unsigned int cmd, unsigned long arg)
{
	...
	switch (cmd) {
		case HN_DPRAM_PHONE_ON:
			printk("HN_DPRAM_PHONE_ON\n");
			dpram_phone_on();
			return 0;

		case HN_DPRAM_PHONE_OFF:
			printk("HN_DPRAM_PHONE_OFF\n");
			dpram_phone_off();
			return 0;
		...
		case HN_DPRAM_PHONE_RAMDUMP:
			dpram_phone_ramdump();
			return 0;

		case HN_DPRAM_PHONE_RESET:
			printk("HN_DPRAM_PHONE_RESET\n");
			dpram_phone_reset();
			return 0;

		case HN_DPRAM_PHONE_BOOTTYPE:
		{
...
#if 0	
#if (CONFIG_NOWPLUS_REV >= CONFIG_NOWPLUS_REV01_N03)
	if(hw_revision != CONFIG_NOWPLUS_REV06 && hw_revision != CONFIG_NOWPLUS_REV07){
		printk(KERN_INFO"[DPRAM] alarm boot pin status : %d\n", omap_get_gpio_datain(OMAP3430_GPIO_ALARM_AP));
		if(status==MESG_PHONE_OFF && !omap_get_gpio_datain(OMAP3430_GPIO_ALARM_AP))
		{
...
static int register_interrupt_handler(void)
{
	...
	retval = request_irq(phone_active_irq, phone_active_interrupt, IRQF_DISABLED, "Phone Active", NULL);
...
void enable_dpram_pins(void)
{
	...
	gpio_direction_output( OMAP_GPIO_FONE_ON, 1 );
	gpio_direction_output( OMAP_GPIO_MSM_RST18_N, 1 );
