...
//#define LOCK_VMA    //???taken from i9003

#define DBGPRINTF( fmt, ...) printk("[OMAP34XXCAM]%s:%d: " fmt "\n", __FUNCTION__,__LINE__, ##__VA_ARGS__) 
...
//#define LCD_REFRESH_CHANGE


#ifdef LCD_REFRESH_CHANGE
extern unsigned short intc refresh_rate;
#endif

//unsigned short int refresh_rate;
...
#if PRINT_FPS 
static int fps=0, fps_cnt=0, fps_sum=0, fps_aver=15;
struct timer_list fps_timer;

void fps_timer_handler(unsigned long);

void fps_timer_init(void)
{
	init_timer(&fps_timer);	
	fps_timer.expires = jiffies + msecs_to_jiffies(1000);    
	fps_timer.function = fps_timer_handler;
	add_timer(&fps_timer);
}
void fps_timer_handler(unsigned long arg)
{	
	fps_cnt++;
	fps_sum += fps;
	printk(KERN_INFO"FPS : (%d )\n", fps);
...
DBGPRINTF("dev=%p", vdev);
  ...
  if(isp_try_fmt_cap(best_pix_in, wanted_pix_out) < 0)
  {
...
//				dev_info(&vdev->vfd->dev, "rs:frmi fmt %8.8x \n", frmi.pixel_format);
...
DBGPRINTF("best_pix: w=%d, h=%d", best_pix->width, best_pix->height);
DBGPRINTF("pix: w=%d, h=%d", pix->width, pix->height);
...
*/
    DBGPRINTF("format.type=%d", f->type);
    DBGPRINTF("format.fmt.pix.width=%d", f->fmt.pix.width);
    DBGPRINTF("format.fmt.pix.height=%d", f->fmt.pix.height);
    DBGPRINTF("format.fmt.pix.pixelformat=0x%x", f->fmt.pix.pixelformat);
    
    DBGPRINTF("V4L2_BUF_TYPE_VIDEO_CAPTURE=%d", V4L2_BUF_TYPE_VIDEO_CAPTURE); 
    DBGPRINTF("V4L2_PIX_FMT_UYVY=0x%x", V4L2_PIX_FMT_UYVY); 
...
DBGPRINTF("in");
	...
	mutex_lock(&vdev->mutex);
	if (vdev->streaming) {
		printk("vidioc_streamon is busy!!");
		rval = -EBUSY;
		goto out;
	}
   
isp_start();    //from i9003

	rval = videobuf_streamon(&ofh->vbq);
	if (rval)
	{
isp_stop();//from i9003
		printk("videobuf_streamon is failed!!");
...
static int vidioc_g_exif(struct file *file, void *fh, struct v4l2_exif *a)
{
	//printk("KKL::vidioc_g_exif is called\n");
			     ...
			     struct v4l2_input *inp)
{
	...
	*/
	...
	struct omap34xxcam_videodev *dev=(struct omap34xxcam_videodev *)fh;
DBGPRINTF("in");
	if(inp->index>=2)
		return -EINVAL;
	
	inp->type = OMAP34XXCAM_SLAVE_SENSOR;
	//printk("name =%s\n",dev->slave[0]->name);
	strlcpy(inp->name, dev->slave[0]->name, sizeof(inp->name));
...
DBGPRINTF("in");
	for (x = 0; x < OMAP34XXCAM_VIDEODEVS; x++) 
	{
		if (cam->vdevs[x].vfd
	    		&& cam->vdevs[x].vfd->minor ==
	    	iminor(file->f_dentry->d_inode)) 
		{
			//printk("matched minor number\n");
			*i=x;
			break;
		}
	}
	//printk("vidioc_g_input exit =%d\n",*i);
...
{
	//printk("omap34xxcam vidioc_s_input =%d\n",i);
	struct omap34xxcam_videodev *vdev=NULL;
	struct omap34xxcam_device *cam=omap34xxcam;
	struct omap34xxcam_fh *fih;
...
DBGPRINTF("in");
	...
	fih = kzalloc(sizeof(*fih), GFP_KERNEL);
	if (fh == NULL)
		return -ENOMEM;
	...
	file->private_data = fih;

	spin_lock_init(&fih->vbq_lock);

	videobuf_queue_sg_init(&fih->vbq, &omap34xxcam_vbq_ops, NULL,
				&fih->vbq_lock, V4L2_BUF_TYPE_VIDEO_CAPTURE,
				V4L2_FIELD_NONE,
