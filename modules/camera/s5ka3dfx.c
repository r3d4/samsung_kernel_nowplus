...
#ifdef CONFIG_MACH_NOWPLUS ////// PROJECT DIVISTION START//////
#include "s5ka3dfx_tune.h"
#elif defined CONFIG_MACH_OSCAR ///// PROJECT DIVISTION ///////////
#include "s5ka3dfx_tune_oscar.h"
#elif defined CONFIG_MACH_HALO ////// PROJECT DIVISTION ///////////
#include "s5ka3dfx_tune_halo.h"
...
static int reg_init_ivs_qcif_index;
static int reg_init_cif_index;
static int reg_init_ivs_cif_index;
static int reg_init_qvga_index;
static int reg_init_ivs_qvga_index;
static int reg_init_vga_index;
static int reg_init_qcif_vt_index;
static int reg_init_ivs_qcif_vt_index;
static int reg_init_cif_vt_index;
static int reg_init_ivs_cif_vt_index;
static int reg_init_qvga_vt_index;
static int reg_init_ivs_qvga_vt_index;
...
static int s5ka3dfx_i2c_write_read(struct i2c_client *client, u8 writedata_num, const u8* writedata, u8 readdata_num, u8* readdata)
{
  ...
  for(i = 0; i < 5; i++)  
  {
    temp =  i2c_transfer(client->adapter, msg, 1);
    if(!(err = (temp == 1 ? 0 : -EINVAL))) 
    {
      break;
    }
    else
    {
      if((i == 4) || (temp == -110 || temp == -5))
      {
        printk(S5KA3DFX_MOD_NAME "s5ka3dfx_i2c_write_read is failed and power off\n");
...
static int s5ka3dfx_i2c_write(struct i2c_client *client, unsigned char length, u8 readdata[])
{
  ...
  for (i = 0; i < 5; i++)
  {
    ...
    else
    {
      if((i == 4) || (temp == -110 || temp == -5))
      {
        printk(S5KA3DFX_MOD_NAME "s5ka3dfx_i2c_write is failed and power off\n");
        sensor->pdata->power_set(V4L2_POWER_OFF);
        return temp;
      } 
    }
...
static int s5ka3dfx_set_table (void)
{
  dprintk(CAM_INF, S5KA3DFX_MOD_NAME "s5ka3dfx_set_table is called...\n");
    ...
    reg_init_ivs_qcif_index = 0;		
    reg_init_cif_index = 0;
    reg_init_ivs_cif_index = 0;		
    reg_init_qvga_index = 0;
    reg_init_ivs_qvga_index = 0;		
    reg_init_vga_index = 0;
    reg_init_qcif_vt_index = 0;
    reg_init_ivs_qcif_vt_index = 0;
    reg_init_cif_vt_index = 0;
    reg_init_ivs_cif_vt_index = 0;
    reg_init_qvga_vt_index = 0;
    reg_init_ivs_qvga_vt_index = 0;
    ...
    reg_init_ivs_qcif_index = sizeof(reg_init_ivs_qcif_table)/sizeof(u32);
    reg_init_cif_index = sizeof(reg_init_cif_table)/sizeof(u32);
    reg_init_ivs_cif_index = sizeof(reg_init_ivs_cif_table)/sizeof(u32);
    reg_init_qvga_index = sizeof(reg_init_qvga_table)/sizeof(u32);
    reg_init_ivs_qvga_index = sizeof(reg_init_ivs_qvga_table)/sizeof(u32);
    reg_init_vga_index = sizeof(reg_init_vga_table)/sizeof(u32);
    reg_init_qcif_vt_index = sizeof(reg_init_qcif_vt_table)/sizeof(u32);
    reg_init_ivs_qcif_vt_index = sizeof(reg_init_ivs_qcif_vt_table)/sizeof(u32);
    reg_init_cif_vt_index = sizeof(reg_init_cif_vt_table)/sizeof(u32);
    reg_init_ivs_cif_vt_index = sizeof(reg_init_ivs_cif_vt_table)/sizeof(u32);
    reg_init_qvga_vt_index = sizeof(reg_init_qvga_vt_table)/sizeof(u32);
    reg_init_ivs_qvga_vt_index = sizeof(reg_init_ivs_qvga_vt_table)/sizeof(u32);
...
static int s5ka3dfx_set_init(void)
{
  ...
  if(sensor->mode == S5KA3DFX_MODE_VT)
  {
    switch(sensor->preview_size)
    {
      ...
      case S5KA3DFX_PREVIEW_SIZE_240_320:
        for (i = 0; i < reg_init_ivs_qvga_vt_index; i++) 
        {
          if(s5ka3dfx_set_data(client, reg_init_ivs_qvga_vt_table[i]))
            goto init_fail;
        }
        break;		
      ...
      case S5KA3DFX_PREVIEW_SIZE_288_352:
        for (i = 0; i < reg_init_ivs_cif_vt_index; i++) 
        {
          if(s5ka3dfx_set_data(client, reg_init_ivs_cif_vt_table[i]))
            goto init_fail;
        }
        break;		
      ...
      case S5KA3DFX_PREVIEW_SIZE_144_176:
        for (i = 0; i < reg_init_ivs_qcif_vt_index; i++) 
        {
          if(s5ka3dfx_set_data(client, reg_init_ivs_qcif_vt_table[i]))
            goto init_fail;
        }
        break; 
  ...
  else
  {
    switch(sensor->preview_size)
    {
      ...
      case S5KA3DFX_PREVIEW_SIZE_240_320:
        for (i = 0; i < reg_init_ivs_qvga_index; i++) 
        {
          if(s5ka3dfx_set_data(client, reg_init_ivs_qvga_table[i]))
            goto init_fail;
        }
        break;
      ...
      case S5KA3DFX_PREVIEW_SIZE_288_352:
        for (i = 0; i < reg_init_ivs_cif_index; i++) 
        {
          if(s5ka3dfx_set_data(client, reg_init_ivs_cif_table[i]))
            goto init_fail;
        }
        break;
      ...
      case S5KA3DFX_PREVIEW_SIZE_144_176:
        for (i = 0; i < reg_init_ivs_qcif_index; i++) 
        {
          if(s5ka3dfx_set_data(client, reg_init_ivs_qcif_table[i]))
            goto init_fail;
        }
        break;
...
static u32 s5ka3dfx_insert_register_table(char* line)
{
  ...
  else if ( parsing_section == REG_INIT_QCIF_VT_SECTION)      reg_init_cif_vt_table[reg_init_qcif_vt_index++] = reg_val;
  else if ( parsing_section == REG_INIT_CIF_VT_SECTION)       reg_init_qcif_vt_table[reg_init_cif_vt_index++] = reg_val;
...
static u32 s5ka3dfx_parsing_section(char* line)
{
  if ( strstr(line, CAMIF_SET_SENSOR_QCIF_INIT) != NULL )
    ...
    parsing_section = REG_FPS_7_SECTION;    
  else if ( strstr(line, CAMIF_SET_SENSOR_FPS10) != NULL )
    parsing_section = REG_FPS_10_SECTION; 
  else if ( strstr(line, CAMIF_SET_SENSOR_FPS12) != NULL )
    parsing_section = REG_FPS_12_SECTION;    
  else if ( strstr(line, CAMIF_SET_SENSOR_FPS15) != NULL )
    parsing_section = REG_FPS_15_SECTION;   
  else
    return -EINVAL;
