...
int P_dev_init(struct i2c_client *client)
{
	...
	printk("============================================\n");
	printk("[SENSOR] P_dev_init  (P_dev.c)              \n");
	printk("============================================\n");
    ...
    /*ldo will remain in default state if we shut it down without having powered it up, hence the following up->down sequence*/
	...
	input_set_abs_params(P_dev.inputdevice, ABS_DISTANCE, 0, 1, 0, 0);
	P_dev.inputdevice->name = "proximity";
	
    if((ret = input_register_device(P_dev.inputdevice))<0) 
    {
        printk( "P_dev_init : input_register_device Fail!! \n") ;
        failed(4);
    }
    else
    {
...
int P_dev_exit(void)
{
	...
	input_unregister_device(P_dev.inputdevice);	// [[ ryun / for input device
...
int P_dev_get_mode(u16 *mode)
{
    ...
    LOCK();   
 	debug("P_dev_get_mode() P_dev.device_state = %d \n", P_dev.device_state);
...
static int vaux1_ldo_up(void)
{
    ...
    //trace_in();
    debug("[PROX] power up Proximity/Light chip....\n");
...
static int vaux1_ldo_down(void)
{
    ...
    debug("[PROX] power down Proximity/Light chip....\n");
    pl_sensor_power_off();
...
static DEVICE_ATTR(poll_delay, 0664, P_delay_show, P_delay_store);
static DEVICE_ATTR(enable, 0664, P_enable_show, P_enable_store);
...
static ssize_t P_enable_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if(enable)
	{
		if((ret = P_dev_powerup_set_op_mode(P_MODE_B)) < 0)
		{
			printk(KERN_ERR "[PROXIMITY] %s : Can't turn on proximity(%d)!\n", __func__, ret);
			return ret;
		}
	} else {
		if((ret = P_dev_shutdown()) < 0)
		{
			printk(KERN_ERR "[PROXIMITY] %s : Can't turn off proximity(%d)!\n", __func__, ret);
		}
	}
...
static ssize_t P_delay_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	...
	P_dev.delay = poll_delay / 1000000;
