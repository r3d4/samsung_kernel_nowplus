...
static int P_ioctl(struct inode *inode, struct file *filp, unsigned int ioctl_cmd, unsigned long arg)
{
    ...
    // trace_in(); 
	debug("[PROX] P_IOC_GET_PROX_OP=0x%x , ioctl_cmd=0x%x \n", P_IOC_GET_PROX_OP, ioctl_cmd);
    ...
    switch (ioctl_cmd)
    {
        case P_IOC_POWERUP_SET_MODE:
            {
                ...
                {
                    debug("[PROX] mode =  %d , argp=%d, P_MODE_B=%d \n", mode, (int)argp, P_MODE_B);
                    ret = -EFAULT;
                    failed(1);
                }
        ...
        case P_IOC_GET_PROX_OP:
            {
                ...
                {
                	debug("[PROX] P_ioctl() : failed(5) ret=%d \n", ret);
                    failed(5);
                }
        ...
        case P_IOC_WAIT_ST_CHG:
            {
                ...
                debug("[P_IOC_WAIT_ST_CHG] mode=%d, P_MODE_B=%d \n", mode, P_MODE_B);
           	    if( mode != P_MODE_B )
           	    {
           	        failed(8);
           	        ret = -EPERM;
                ...
                debug("[P_IOC_WAIT_ST_CHG] init_wait( &(fpriv->waitq_entry) ); \n");
                ...
                {
                    debug("[P_IOC_WAIT_ST_CHG] schedule(); +  \n");
                    schedule();
                    debug("[P_IOC_WAIT_ST_CHG] schedule(); +  \n");
                }
        ...
        default:
            debug("%s Inappropriate ioctl default 0x%x", __FUNCTION__, ioctl_cmd);
            ret = -ENOTTY;
            break;
    }
...
static irqreturn_t P_isr( int irq, void *unused )
{
	...
	debug("[PROX] Proximity interrupt!!");

    // schedule_work(&P_ws);
	ret = schedule_work(&P_ws);
	debug("[PROX] schedule_work(&P_ws) ret(1:succes, 0:fail) = %d", ret );
...
void P_enable_int(void)
{
    debug("PROX INTERRUPT ENABLED");
    enable_irq(P_IRQ);
}

void P_disable_int(void)
{
    debug("PROX INTERRUPT DISABLED");
    disable_irq(P_IRQ);
}
...
static void GPIO_setting( void )
{
	...
	void __iomem*   gpio28_pad_config_reg;
	u32 reg_val;
...
#if defined(CONFIG_MACH_SAMSUNG_ARCHER) && (CONFIG_ARCHER_REV >= ARCHER_REV11)
	gpio28_pad_config_reg = ioremap(0x480025F8, 4);
    reg_val = __raw_readl( gpio28_pad_config_reg );
    
	reg_val &= 0xFFFFFEE0; // save origin val
	reg_val |= 0x0000011C; // input En, PullType UP, Pull En, Muxmod 4 : GPIO

	__raw_writel( reg_val, gpio28_pad_config_reg );
#else 
    //	gpio53_pad_config_reg = ( void __iomem * )IO_ADDRESS( 0x480025F4 );      // 0x480020B0 : gpio_27
    //	gpio53_pad_config_reg = ( void __iomem * )IO_ADDRESS( P_IRQ );           // 0x480020B0 : gpio_53
	gpio28_pad_config_reg = ioremap(0x480020B0, 4);
	//gpio53_pad_config_reg = ( void __iomem * )0x480020B0;  // 0x480020B0 : gpio_53
	reg_val = __raw_readl( gpio53_pad_config_reg );

    //	reg_val &= 0xFFF8FFFF; // save origin val
    //	reg_val |= 0x00040000; // Muxmod 4 : GPIO
	reg_val &= 0xFEE0FFFF; // save origin val
	reg_val |= 0x011C0000; // input En, PullType UP, Pull En, Muxmod 4 : GPIO

    //	gpio_direction_input(0x480020B0);	// ryun
