...
#elif CONFIG_SAMSUNG_ARCHER_TARGET_SK
...
#ifdef  CONFIG_SAMSUNG_ARCHER_TARGET_SK  // Archer_LSJ DA27
...
static int KXSD9_open (struct inode *inode, struct file *filp)
{
    ...
    KXSD9_dev_set_operation_mode(ACCEL, 1);
    trace_out();
#endif
    return ret;
}
...
#else
    trace_in();
    KXSD9_dev_set_operation_mode(ACCEL, 0);
    trace_out();
#endif
    return 0;
}
...
static int KXSD9_ioctl(struct inode *inode, struct file *filp, unsigned int ioctl_cmd,  unsigned long arg)
{
    ...
    switch (ioctl_cmd)
    {
        case KXSD9_IOC_SET_DELAY:
            {                
                ret= copy_from_user( (void*)&delay, argp, sizeof(int) ) ;
                if( ret < 0 )
                {
        ...
        case KXSD9_IOC_GET_INITIAL_VALUE:  
            {
                ...
                if( copy_to_user( (kxsd9_convert_t*)arg, &kxsd9_convert, sizeof(kxsd9_convert_t) ) )
                {
                    ret = -EFAULT;
                }   
        ...
        case KXSD9_IOC_GET_ACC:
            {
                ...
                #if 1  // for debug 
                if (Accel_log_debug_cnt == 50)
                {
                    Accel_log_debug_cnt = 0; 
                    printk("[ACCEL] X:%d, Y:%d, Z:%d \n", latest_acc.X_acc, latest_acc.Y_acc, latest_acc.Z_acc) ;
                }
                Accel_log_debug_cnt = Accel_log_debug_cnt +1; 
                #endif 
            }
            break;
...
int KXSD9_timer_disable( void )
{
    ...
    {
        printk( "[ACCEL] Timer OFF already!!!\n" ) ;
        trace_out();
        return KXSD9_timer.current_timer_state ;
    }   
...
static DEVICE_ATTR(calibration, 0664, KXSD9_calibration_show, KXSD9_calibration_store);
...
int __init KXSD9_driver_init(void)
{
	...
	printk("============================================\n");
	printk("[SENSOR] KXSD9_driver_init  (KXSD9_main.c)  \n");
	printk("============================================\n");
    ...
    // ret = device_create_file(&input_dev->dev.kobj, &KXSD9_attribute_group);
    if(ret < 0)
    {
    ...
    {
        printk("\n [ACCEL] sysfs_create_group SUCCESS!!        \n");
    }
