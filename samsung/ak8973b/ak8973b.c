...
#define IRQ_COMPASS_INT OMAP_GPIO_IRQ(OMAP_GPIO_COMPASS_INT)
...
static int ak_proc_read(char *page, char **start, off_t off, int count, int *eof, void *data)
{
	...
	AKECS_SetMeasure();
	while(!gpio_get_value(OMAP_GPIO_COMPASS_INT));
...
static void AKECS_Reset (void)
{
	...
	gpio_set_value(OMAP_GPIO_COMPASS_RST, 0);
	udelay(120);
	gpio_set_value(OMAP_GPIO_COMPASS_RST, 1);
...
{
	if(gpio_is_valid(OMAP_GPIO_COMPASS_RST)){
		if(gpio_request(OMAP_GPIO_COMPASS_RST, NULL))
		{
			printk(KERN_ERR "Failed to request OMAP3430_GPIO_COMPASS_RST!\n");
		}
		gpio_direction_output(OMAP_GPIO_COMPASS_RST, 1);
	}

	if(gpio_is_valid(OMAP_GPIO_COMPASS_INT)){
		if(gpio_request(OMAP_GPIO_COMPASS_INT, NULL))
		{
			printk(KERN_ERR "ECOMPASS: %s: can't get int pin\n", __func__);
			return;
		}
		gpio_direction_input(OMAP_GPIO_COMPASS_INT);
	}
	else
	{
		printk("ECOMPASS: %s: COMPASS_INT is not valid!\n", __func__);
	}
...
static int __exit ak8973_remove(struct i2c_client *client)
{
	...
	gpio_free(OMAP_GPIO_COMPASS_RST);
